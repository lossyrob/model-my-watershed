...
object TR55 {
  ...
  val wordToSoil = collection.immutable.HashMap(
    1603364 -> "A",
    1603365 -> "A/D",
    1603366 -> "A",
    1603367 -> "B/D",
  ...
  def combinations(nlcd: RasterRDD[SpatialKey], soil: RasterRDD[SpatialKey], polygon: Polygon): Map[(Int, Option[String]), Int] = {
    ...
    val histogram = nlcdAndSoil
      .map { case (key, (nlcdTile, soilTile)) =>
        ...
        val localHistogram = mutable.Map[(Int, Option[String]), Int]()

        clippedPolygon match {
          case PolygonResult(p) =>
            Rasterizer.foreachCellByPolygon(p, rasterExtent)(
              new Callback {
                def apply(col: Int, row: Int): Unit = {
                  val soilType = wordToSoil.get(soilTile.get(col,row))
                  if (soilType != None) {
